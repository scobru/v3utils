...
contract V3Utils is IERC721Receiver {
    ...
    function execute(uint256 tokenId, Instructions calldata instructions) external {
        ...
        nonfungiblePositionManager.safeTransferFrom(msg.sender, address(this), tokenId, abi.encode(instructions));
    }
    ...
    function _swapAndMint(
        ...
        bool unwrap
    ) internal returns (uint256 tokenId, uint128 liquidity, uint256 added0, uint256 added1) {
    ...
    function _swap(
        ...
        if (amountIn != 0 && address(tokenOut) != address(0)) {
            // approve needed amount
            SafeERC20.safeApprove(tokenIn, address(swapRouter), amountIn);

            uint256 balanceInBefore = tokenIn.balanceOf(address(this));
            uint256 balanceOutBefore = tokenOut.balanceOf(address(this));

            // prepare calldata for swap
            ...
            // reset approval
            SafeERC20.safeApprove(tokenIn, address(swapRouter), 0);
