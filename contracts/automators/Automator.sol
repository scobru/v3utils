...
abstract contract Automator is Ownable {
    ...
    function _swap(
        ...
        if (amountIn > 0 && swapData.length > 0) {
            ...
            address swapRouter = swapRouterIndex == 0
                ? swapRouterOption0
                : (swapRouterIndex == 1 ? swapRouterOption1 : swapRouterOption2);
    ...
    function _hasMaxTWAPTickDifference(
        IUniswapV3Pool pool,
        uint32 twapPeriod,
        int24 currentTick,
    ...
    function _getPool(address tokenA, address tokenB, uint24 fee) internal view returns (IUniswapV3Pool) {
        return
            IUniswapV3Pool(PoolAddress.computeAddress(address(factory), PoolAddress.getPoolKey(tokenA, tokenB, fee)));
    }
