...
contract AutoRange is Automator {
    ...
    function execute(ExecuteParams memory params) external {
        ...
        (state.amount0, state.amount1) = _decreaseFullLiquidityAndCollect(
            params.tokenId,
            state.liquidity,
        ...
        if (
            ...
            (state.amountInDelta, state.amountOutDelta) = _swap(
            ...
            if (state.amount0 - state.protocolReward0 - state.amountAdded0 > 0) {
                _transferToken(
                    state.owner,
                    IERC20(state.token0),
                    state.amount0 - state.protocolReward0 - state.amountAdded0,
                    true
                );
            }
            if (state.amount1 - state.protocolReward1 - state.amountAdded1 > 0) {
                _transferToken(
                    state.owner,
                    IERC20(state.token1),
                    state.amount1 - state.protocolReward1 - state.amountAdded1,
                    true
                );
            }
